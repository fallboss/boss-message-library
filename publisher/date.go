...
func ParseInLocation(date string, country string) string {
	if "" != date {
		...
		if nil != er {
			logg.Errorf("error occurred inf parse date for: %s", er)
			return ""
		}
		...
		if strings.Contains(date, "/") {
			...
			if err != nil {
				logg.Error("Error parsing date: " + date + " in location: " + GetTimeZone(country) + " error: " + err.Error())
				return ""
			}
...
func GetLocalDate(date string, country string) time.Time {
	...
	location, er := time.LoadLocation(GetTimeZone(country))
	if er != nil {
		logg.Errorf("error load Location : %s", er)
	}
	if strings.Contains(date, "/") {
		inLocation, err := time.ParseInLocation("01/02/2006 15:04:05", date, location)
		if err != nil {
			logg.Errorf("error parsing date : %s", err)
		}
